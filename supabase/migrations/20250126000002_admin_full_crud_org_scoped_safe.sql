-- Migration: Grant Admin Role Full CRUD Privileges Scoped to Their Organization (SAFE VERSION)
-- This version checks if tables exist before applying policies
-- Run this INSTEAD of 20250126000000_admin_full_crud_org_scoped.sql
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'timesheet_change_requests') THEN
    
    EXECUTE 'DROP POLICY IF EXISTS "Employees can view their own timesheet change requests" ON public.timesheet_change_requests';
    EXECUTE 'DROP POLICY IF EXISTS "Employees can create their own timesheet change requests" ON public.timesheet_change_requests';
    EXECUTE 'DROP POLICY IF EXISTS "Admins can view timesheet change requests in their organization" ON public.timesheet_change_requests';
    EXECUTE 'DROP POLICY IF EXISTS "Admins can update timesheet change requests in their organization" ON public.timesheet_change_requests';
    EXECUTE 'DROP POLICY IF EXISTS "Allow authenticated access to timesheet_change_requests" ON public.timesheet_change_requests';

    EXECUTE 'CREATE POLICY "Owners can view all change requests" ON public.timesheet_change_requests FOR SELECT USING (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can view change requests in their org" ON public.timesheet_change_requests FOR SELECT USING (is_admin() AND (organization_id = current_user_organization_id() OR organization_id IS NULL))';
    EXECUTE 'CREATE POLICY "Employees can view own change requests" ON public.timesheet_change_requests FOR SELECT USING (employee_id IN (SELECT id FROM public.employees WHERE staff_id = (auth.jwt() ->> ''username''::text)))';
    EXECUTE 'CREATE POLICY "Employees can create own change requests" ON public.timesheet_change_requests FOR INSERT WITH CHECK (employee_id IN (SELECT id FROM public.employees WHERE staff_id = (auth.jwt() ->> ''username''::text)))';
    EXECUTE 'CREATE POLICY "Owners can update all change requests" ON public.timesheet_change_requests FOR UPDATE USING (is_owner()) WITH CHECK (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can update change requests in their org" ON public.timesheet_change_requests FOR UPDATE USING (is_admin() AND (organization_id = current_user_organization_id() OR organization_id IS NULL)) WITH CHECK (is_admin() AND (organization_id = current_user_organization_id() OR organization_id IS NULL))';
    EXECUTE 'CREATE POLICY "Owners can delete change requests" ON public.timesheet_change_requests FOR DELETE USING (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can delete change requests in their org" ON public.timesheet_change_requests FOR DELETE USING (is_admin() AND (organization_id = current_user_organization_id() OR organization_id IS NULL))';
    
    RAISE NOTICE 'Applied RLS policies to timesheet_change_requests table';
  ELSE
    RAISE NOTICE 'Skipped timesheet_change_requests table (does not exist)';
  END IF;
END $$;
...
-- ============================================================================
-- 8. EMPLOYEE_ROLES TABLE - Full CRUD for admins in their org (OPTIONAL)
-- ============================================================================

-- Only apply if table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employee_roles') THEN
    
    EXECUTE 'DROP POLICY IF EXISTS "Enable read access for all users" ON public.employee_roles';
    EXECUTE 'DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON public.employee_roles';
    EXECUTE 'DROP POLICY IF EXISTS "Enable update for authenticated users only" ON public.employee_roles';
    EXECUTE 'DROP POLICY IF EXISTS "Enable delete for authenticated users only" ON public.employee_roles';

    EXECUTE 'CREATE POLICY "Owners can view all employee roles" ON public.employee_roles FOR SELECT USING (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can view employee roles in their org" ON public.employee_roles FOR SELECT USING (is_admin() AND organization_id = current_user_organization_id())';
    EXECUTE 'CREATE POLICY "Users can view employee roles" ON public.employee_roles FOR SELECT USING (organization_id = current_user_organization_id())';
    EXECUTE 'CREATE POLICY "Owners can create employee roles" ON public.employee_roles FOR INSERT WITH CHECK (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can create employee roles in their org" ON public.employee_roles FOR INSERT WITH CHECK (is_admin() AND organization_id = current_user_organization_id())';
    EXECUTE 'CREATE POLICY "Owners can update all employee roles" ON public.employee_roles FOR UPDATE USING (is_owner()) WITH CHECK (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can update employee roles in their org" ON public.employee_roles FOR UPDATE USING (is_admin() AND organization_id = current_user_organization_id()) WITH CHECK (is_admin() AND organization_id = current_user_organization_id())';
    EXECUTE 'CREATE POLICY "Owners can delete employee roles" ON public.employee_roles FOR DELETE USING (is_owner())';
    EXECUTE 'CREATE POLICY "Admins can delete employee roles in their org" ON public.employee_roles FOR DELETE USING (is_admin() AND organization_id = current_user_organization_id())';
    
    RAISE NOTICE 'Applied RLS policies to employee_roles table';
  ELSE
    RAISE NOTICE 'Skipped employee_roles table (does not exist)';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'owner_organization_access') THEN
    
    EXECUTE 'DROP POLICY IF EXISTS "Owners can manage their organization access" ON public.owner_organization_access';
    EXECUTE 'DROP POLICY IF EXISTS "Owners can manage all organization access" ON public.owner_organization_access';
    EXECUTE 'DROP POLICY IF EXISTS "Allow organization access management" ON public.owner_organization_access';

    EXECUTE 'CREATE POLICY "Owners can manage org access" ON public.owner_organization_access FOR ALL USING (is_owner()) WITH CHECK (is_owner())';
    
    RAISE NOTICE 'Applied RLS policies to owner_organization_access table';
  ELSE
    RAISE NOTICE 'Skipped owner_organization_access table (does not exist)';
  END IF;
END $$;
...
SELECT 'Helper functions created: is_owner(), is_admin(), current_user_organization_id(), is_admin_or_owner()' as info;
SELECT 'RLS policies applied to: admin_users, organizations, employees, timesheet_entries, wage_settings, company_settings' as info;
SELECT 'Optional tables (if exist): timesheet_change_requests, employee_roles, owner_organization_access' as info;
